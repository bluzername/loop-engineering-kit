...
A discovery-and-classification pass meant to run on a **schedule** (the `/schedule` skill, a scheduled task, or an external cron running `claude -p`) so the
...
Use the `/schedule` skill (or your scheduler of choice) to run `/triage <target> <level>`. Keep `prs` out of the schedule - use
...
schedule `/triage <target>` with `/schedule`."* Log the run to `run-log.md` if the
